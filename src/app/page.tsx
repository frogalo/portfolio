...
export default function HomePage() {
        ...
        switch (heroState) {
            case 0:
                return {
                    ...
                    color: "text-[var(--nav-btn-color)]" // Or specific purple? Let's use specific hexes or classes if needed, or mapped logic
                };
            case 1:
                return {
    ...
    const getHeroColorClass = (state: number) => {
        switch(state) {
             case 0: return "text-[var(--primary)]"; // Creative - Primary (Purple)
             case 1: return "text-[var(--accent)]"; // Network - Accent (Pink/Red)
             case 2: return "text-[#08fdd8]"; // DevOps - Cyan
        }
    };
    ...
    return (
        <div className="bg-background min-h-screen text-text selection:bg-accent selection:text-background font-sans overflow-x-hidden">
            ...
            <section ref={targetRef} className="relative min-h-screen flex flex-col justify-center px-4 md:px-8 pt-20">
                <motion.div style={{ opacity, scale }} className="w-full max-w-[1920px] mx-auto z-10">
                    <div className="flex flex-col uppercase leading-[0.85] tracking-tighter">
                         <motion.h1 
                            initial={{ y: 100, opacity: 0 }}
                            animate={{ y: 0, opacity: 1 }}
                            transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
                            className="text-[min(11vw,15vh)] lg:text-[8vw] font-bold text-text mb-0 pb-0" 
                         >
                            JAKUB
                        </motion.h1>
                        <motion.h1 
                    ...
                    <div className="flex flex-col md:flex-row justify-end mt-12 md:mt-24 gap-12">
                         <motion.div 
                            ...
                            className="w-full md:w-1/3 text-lg md:text-xl font-light leading-relaxed text-primary/80"
                         >
                            <p className="mb-6">
                                {t("heroIntro1")}
                            </p>
